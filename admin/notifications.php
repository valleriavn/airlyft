...
$tables = ($type_filter === 'email') ? "EmailNotification" : (($type_filter === 'sms') ? "SMSNotification" :
...
$sql = "
    SELECT 
        ...
        b.total_amount, CONCAT(u.first_name, ' ', u.last_name) AS customer_name, p.place_name AS destination
    FROM $tables n
                            ...
                            <?php else: ?>
                                <?php while ($row = $notifications->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge bg-<?=
                                                                    $row['status'] === 'Sent' ? 'success' : ($row['status'] === 'Pending' ? 'warning' : 'danger') ?>">
                                                <?= $row['status'] ?>
                                            </span>
                                        </td>
                                        <td>
